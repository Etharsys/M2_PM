...
void FluidGrid2::set_bnd(int bound, Grid& x)
{
    ...
    for (int i = 1; i < side() - 1; ++i)
    {
        ...
        x[side()-1][i] = bound == 1 ? -x[side()-2][i] : x[side()-2][i];
        x[i][0       ] = bound == 2 ? -x[i][1       ] : x[i][1       ];
        x[i][side()-1] = bound == 2 ? -x[i][side()-2] : x[i][side()-2];
        
    }

    x[0       ][0       ] = .5 * (x[1       ][0       ] + x[0       ][1       ]);
    x[0       ][side()-1] = .5 * (x[1       ][side()-1] + x[0       ][side()-2]);
    x[side()-1][0       ] = .5 * (x[side()-2][0       ] + x[side()-1][1       ]);
    x[side()-1][side()-1] = .5 * (x[side()-2][side()-1] + x[side()-1][side()-2]);
}


void FluidGrid2::diffuse(int bound, Grid& x, const Grid& x0, int iter)
{
    double a = _lifespan * _diffusion * size();
