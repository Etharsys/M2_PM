...
namespace fluid3d
{
    ...
    FluidData::FluidData(int _N, float _diff, float _visc, float _dt)
    {
        N = _N;
    ...
    void FluidData::set_bnd(int b, float *x)
    {
        for(int j = 1; j < N - 1; j++) {
            for(int i = 1; i < N - 1; i++) {
    ...
    void FluidData::lin_solve(int b, float *x, float *x0, float a, float c)
    {
        float cRecip = 1.0 / c;
    ...
    void FluidData::diffuse (int b, float *x, float *x0, float environment)
    {
        float a = dt * environment * (N - 2) * (N - 2);
        lin_solve(b, x, x0, a, 1 + 6 * a);
    }

    void FluidData::advect (int b, float *d, float *d0, float *velocX, float *velocY, float *velocZ)
    {
    ...
    void FluidData::project(float *velocX, float *velocY, float *velocZ, float *p, float *div)
    {
        for (int k = 1; k < N - 1; k++) {
            for (int j = 1; j < N - 1; j++) {
                for (int i = 1; i < N - 1; i++) {
    ...
    void FluidData::FluidDataAddDensity(int x, int y, int z, float amount)
    {
        density[IX(x, y, z)] += amount;
    }

    void FluidData::FluidDataAddVelocity(int x, int y, int z, float amountX, float amountY, float amountZ)
    {
        int index = IX(x, y, z);
    ...
    std::vector<float> FluidData::flat() const
    {
        std::vector<float> res;
        for (int i = 0; i < N*N*N; i++)
        {
            res.emplace_back(density[i]);
        }
        return res;
    }
